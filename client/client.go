...
func StartClient(conn grpc.ClientConnInterface, messageSends <-chan string, messageReceives chan<- *service.UserMessage) {
	...
	go listenForMessages(stream, messageReceives)
	go messageSender(stream, messageSends)
}

func messageSender(stream service.Chittychat_ChatSessionClient, messages <-chan string) {
	for {
...
func listenForMessages(stream service.Chittychat_ChatSessionClient, messagesChannel chan<- *service.UserMessage) {
